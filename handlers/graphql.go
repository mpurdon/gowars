...
func (h GraphQL) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if ok := isSupported(r.Method); !ok {
		h.Logger.Debug().
			Str("method", r.Method).
			Msg("request method is not supported")
	...
	//
	// We don't need it for this application, but a typical application would
